...
/**
 * Servlet implementation class UpdateClassTeach
 */
@WebServlet("/UpdateClassTeach")
public class UpdateClassTeach extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public UpdateClassTeach() {
        super();
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		int TeaId = Integer.parseInt(request.getParameter("Teachid"));
		int status=0;
		try {
			...
			PreparedStatement ps = con.prepareStatement("update subject set Cid=? , tid=? where Subid=?");
			ps.setInt(1,Clsid);
			ps.setInt(2, TeaId);
			ps.setInt(3, Subid);
